...
3. 每关页面的「专属提示码」区域按玩家签发**一次性码**（每人每码唯一，10 分钟有效，用完即焚），
   全部通过统一指令 `私聊汐月 /submit 0x<码>` 兑换（码自带身份，服务器按码兑付）：
   - 每关 3 层提示：第 1 层等 5 分钟解锁、第 2 层等 20 分钟解锁（自首次打开本关页起算）、第 3 层**点击申请、管理员在后台审批**后发放（兑换均需通关前一关）
   - 记忆库开启码（第 5 关，需完成前 4 关）
   - 凭证码（第 7 关，需完成前 6 关）
   - 彩蛋码（终局成功页签发，隐藏结局）
   - `/进度` — 通关进度 + 已用码列表
4. 码自生成起 10 分钟有效，过期回本关页面刷新重新签发；用过即焚，再发会提示"已用"。未到解锁时间会提示"还需等待"。
5. 若在**群聊**里发送 `/submit`，汐月只会提示"只能私聊"，**不会消耗任何码**，不会卡关。
