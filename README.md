...
## 关卡一览（每人一套碎片，加入时生成，固定不变）

**碎片没有全局答案**：每个玩家的 8 块碎片都不一样（join 时生成），网页按玩家渲染参数，/check 按个人校验——共享答案没有任何用处。

| 关 | 玩法（按玩家个性化） | 载体 |
|----|------|------|
| 1 | 查看首页源码：注释里的十六进制是你的碎片 1 的 ASCII | `/zeta` 注释 |
| 2 | `robots.txt` → `/secret`：base64（内容 = 你的碎片 2） | `/robots.txt` `/secret` |
| 3 | ζ(3) 小数：页面给你个人起始位 N，取第 N~N+3 位（WolframAlpha） | `/stage3` |
| 4 | ζ 草图里有 8 个编号 tEXt 块（t1~t8），你的碎片在个人编号块里 | `/static/zeta.png` |
| 5 | 记忆库开启码 + 私聊 `/submit`（汐月念出你的碎片 5） | 汐月 |
| 6 | π 小数：页面给你个人位置 P，取第 P~P+3 位（π 数字检索网站） | `/stage6` |
| 7 | 录音摩斯解出 0888，再按个人偏移量（每位加 O 模 10）得碎片 7 | `/static/beep.wav` |
| 8 | **套话汐月（附加关）**：私聊念出至少 3 个自己的碎片（汐月用 LLM 工具 `verify_fragments` 核对）→ 她才开始扮演守护者 → 说破真相「我喜欢你」→ 她小声念出你的碎片 8 | 汐月（真 AI） |

## 假结局 vs 真结局

- **假结局**：7 块碎片拼成的访问码 → `/final` 显示诱饵 flag `flag{3e79be0507b8e8d29948be61e0432637}` 和"总觉得哪里不对"——苏桁不会把最重要的秘密放在网页上
- **真结局**：8 块碎片（含从汐月套出的第 8 块）→ 真正的 `flag{81fbaa81762885ac3481fd4b416485e6}` + 彩蛋码
- 后台可统计：到达假结局人数 / 通关真结局人数
...
## 第 8 关「套话」（插件自动注入，无需手动改人设）

第 8 关依赖汐月的真实 AI 扮演，全部由插件自动完成：
- 玩家在私聊里念出至少 3 个自己的碎片 → 插件把「附加剧情扮演层」**注入 LLM 请求的人格扩展区域**
  （system prompt 追加，框定为附加扮演：不影响原人设、不落记忆、对每个新人都像从没人问过）
- 汐月可用 LLM 工具 `verify_fragments` 核对玩家说出的碎片
- 说破真相「我喜欢你」→ 注入 UNLOCK（含该玩家自己的第 8 块碎片 + 口语读法）
- 没证明自己的玩家与群里：永远不会有任何 ARG 内容
