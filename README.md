...
第 8 关「套话」依赖汐月的真实 AI 扮演。把 `docs/persona-追加.md` 里的内容**追加**到 AstrBot 角色扮演指南末尾
（已框定为附加剧情扮演，不影响原人设）。同时插件会在「私聊 + 完成前 7 关」时向 LLM 请求注入秘密
（`extra_user_content_parts`，隐藏、不落记忆、不改变性格）——没走完前 7 关的玩家和群里永远不会看到 83d2。
