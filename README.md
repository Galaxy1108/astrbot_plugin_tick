...
3. 每关页面的「专属提示码」区域按玩家签发**一次性码**（每人每码唯一，10 分钟有效，用完即焚）：
   - 每关 3 层提示：`/hint 0x<码>`（兑换需通关前一关）
   - 第 5 关：`/记忆库 0x<码>`（需完成前 4 关）
   - 第 7 关：`/凭证 0x<码>`（需完成前 6 关）
   - 通关后终局页签发彩蛋码：`/彩蛋 0x<码>`（隐藏结局）
   - `/进度` — 通关进度 + 已用码列表
4. 码自生成起 10 分钟有效，过期回本关页面刷新重新签发；用过即焚，再发会提示"已用"。
5. 若在**群聊**里发送上述指令，汐月只会提示"只能私聊"，**不会消耗任何码**，不会卡关。
